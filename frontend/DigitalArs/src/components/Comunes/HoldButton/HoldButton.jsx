// React Bits Hold Button, adaptado para DigitalArs. Ver LICENSE.md.
// https://reactbits.dev/c/micro/hold-button
// Adaptaciones: ayuda en español, cancelación por foco y guard al deshabilitar.
...
export default function HoldButton({
  ...
  const drive = (to, duration, ease) => {
    ...
    m.from = m.p;
    m.to = to;
    // drive solo se ejecuta desde eventos y temporizadores, nunca durante render.
  ...
  const complete = () => {
    ...
    if (!buttonRef.current || buttonRef.current.disabled || document.hidden) return;
    // complete solo corre desde requestAnimationFrame/setTimeout.
    // eslint-disable-next-line react-hooks/purity
    if (performance.now() - gesture.current.start < holdTime) return;
    clearTimers();
  ...
  useLayoutEffect(() => { releaseRef.current = release; });
  ...
  const handlePointerDown = e => {
    ...
    } catch { /* La cancelación por salida sigue disponible sin captura. */ }
  };
  ...
  const endPointer = (e, options) => {
    ...
    } catch { /* El navegador puede haber liberado la captura antes. */ }
    release(options);
  };
  ...
  return (
    ...
    >
      ...
      <span id={hintId} className="hold-button__sr">
        Mantené presionado {Math.round(holdTime / 100) / 10} segundos para confirmar.
        También podés mantener Espacio o Enter. Soltá antes para cancelar.
      </span>
    </button>
  );
}
